-- 0039_super_scout_ratings.sql — add subjective super-scout ratings.
--
-- The review screen now lets a stands scout rate a robot's DRIVER SKILL and
-- AGILITY (0–3 ordinals, 0 = not rated) alongside the existing defense_rating.
-- These are advisory qualitative observations (NOT scored points), stored like
-- defense_rating / foul_reasons — the server NEVER recomputes them, so the
-- scoring sync contract (client/RPC/seed byte-equivalence) is untouched.
--
-- 1. Add two int columns (default 0 so every existing row stays valid).
-- 2. Recreate upsert_match_report to read `driver_skill` + `agility` from the
--    payload, parsed exactly like `defense_rating` (coalesce to 0).
--
-- ADDITIVE / verbatim: this is the 0032 body with ONLY the two columns added to
-- the INSERT column list + VALUES and the UPDATE set. Every other column,
-- coalesce, the scout resolve/provision block, the revision guard, the supersede
-- dedupe, and the grant are preserved unchanged.

alter table match_scouting_report
  add column if not exists driver_skill int not null default 0,
...
begin
  ...
  if v_existing_rev is null then
    ...
    insert into match_scouting_report (
      ...
      intake_sources, max_fuel_capacity_observed, defense_rating, driver_skill, agility, pins, fouls_minor,
      ...
      coalesce((p->>'driver_skill')::int, 0),
      coalesce((p->>'agility')::int, 0),
    ...
    update match_scouting_report set
      ...
      driver_skill = coalesce((p->>'driver_skill')::int, 0),
      agility = coalesce((p->>'agility')::int, 0),
